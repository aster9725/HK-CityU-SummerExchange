...
public class MainActivity extends Activity {
	...
	private void playgame() {
		...
		if (curquestion == 30)
			...
			for (int i = 0; i < 4; i++) {
				offsets[i] = (int) (Math.random() * 100) % 30;
				if(offsets[i] == aryques[curquestion]){
					i--;
					continue;
				}
				else {
			...
			for(int i=0;i<4;i++){
				mBtnAns[i].setBackgroundResource(R.drawable.img_animal_bear+category+offsets[i]);
			}
	...
	private void setTimer() {
		...
		mTimeBar.setMax(4500);
		timeleft = 0;
		cntTimer = new CountDownTimer(50 * 1000, 100) {
	...
	private void savePreferences(int score) {
		...
		for (i = 0; i < 5; i++) {
			switch(lang){
			case STARTENG:
				pref.edit().putInt(prefKey_eng[category/30][i], prefVal[i]).commit();
				break;
			case STARTCHI:
				pref.edit().putInt(prefKey_chi[category/30][i], prefVal[i]).commit();
				break;
			case STARTKOR:
				pref.edit().putInt(prefKey_kor[category/30][i], prefVal[i]).commit();
				break;
			}
	...
	private void loadPreferences() {
		...
		for (int i = 0; i < 5; i++) {
			switch(lang){
			case STARTENG:
				prefVal[i] = pref.getInt(prefKey_eng[category/30][i], -1);
				break;
			case STARTCHI:
				prefVal[i] = pref.getInt(prefKey_chi[category/30][i], -1);
				break;
			case STARTKOR:
				prefVal[i] = pref.getInt(prefKey_kor[category/30][i], -1);
				break;
			}
		}
	}
